...
1. **Полного зеркалирования произвольного UI приложения на CarPlay не
   существует БЕЗ entitlement и без своего видео-окна.** CarPlay — не
   универсальный внешний дисплей. Почти весь UI рендерится системными
   шаблонами (`CPListTemplate`, `CPGridTemplate`, `CPNowPlayingTemplate` и
   т.д.), и **`WKWebView` в CarPlay не встраивается вообще** — то есть
   встроенный браузер на машинный экран попасть не может даже теоретически.
   НО: с выданным Video-entitlement и собственным видео-окном зеркалирование
   всё же возможно — см. факт 7 (прецедент CarTV). Ограничение — не
   архитектурное, а именно entitlement-ное.
...
7. **Прецедент на App Store: CarTV (`cartv.app`, App Store id6800211040)
   реально делает зеркалирование экрана на CarPlay.** Проверено 2026-09-24:
   юзер-флоу — открыть CarTV на телефоне → Пункт управления → «Повтор
   экрана» → в списке уже выбран CarTV (это ReplayKit Broadcast Extension,
   такой же механизм, что уже реализован в
   [BroadcastExtension/SampleHandler.swift](BroadcastExtension/SampleHandler.swift))
   → начать трансляцию → открыть саму CarTV на экране CarPlay → там
   показывается живой поток с телефона, весь экран, любое приложение.
   Требование iOS 18+ (не 26+) и отсутствие жёсткого ОС-ограничения
   «только на парковке» в их маркетинге намекают, что они не проходят
   через новую Video-категорию iOS 26 в чистом виде, либо используют её
   вместе с чем-то ещё — это не до конца ясно, но **факт: связка
   ReplayKit-экстеншн + CarPlay-видео-окно реально одобряется Apple и
   работает на проде.** Значит архитектура, которую заложил Грок
   (`BroadcastExtension` → App Group → окно на CarPlay-сцене), верная по
   направлению — не хватает только выданного entitlement и подключения
   `MirrorFrameStore` к настоящей видео-поверхности вместо `CPListTemplate`
   заглушки в [CarPlaySceneDelegate.swift](Sources/CarPlay/CarPlaySceneDelegate.swift).
   При подаче заявки в Apple (факт 6.1) имеет смысл явно описывать именно
   этот use-case (зеркалирование для пассажира/на парковке) — прецедент на
   сторе есть, значит категория такое одобряет.
